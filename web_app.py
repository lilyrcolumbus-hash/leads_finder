...
def show_search():
    ...
    use_gmaps = st.checkbox("📍 Google Maps", value=True)
    use_yelp = st.checkbox("⭐ Yelp", value=True)
    use_indeed = st.checkbox("💼 Indeed", value=True)
    use_yp = st.checkbox("📒 Yellow Pages", value=True)
    use_bbb = st.checkbox("🏢 BBB", value=True)
    use_cl = st.checkbox("📰 Craigslist", value=True)
    ...
    if st.button("🚀 INICIAR BÚSQUEDA", type="primary", use_container_width=True):
        ...
        if use_yelp:
            scrapers.append(("Yelp", YelpScraper))
        if use_indeed:
            scrapers.append(("Indeed", IndeedScraper))
        if use_yp:
            scrapers.append(("Yellow Pages", YellowPagesScraper))
        if use_bbb:
            scrapers.append(("BBB", BBBScraper))
        if use_cl:
